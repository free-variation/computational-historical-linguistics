...
% lex(DB, ID, LanguageID, ParameterID, Segments, GlottologName, GlottoCode, Family, 
% ConceptID, Gloss, CognatesetID, CC, ASJP)
concept_set_with_cognates(Family, ConceptID, ConceptSet) :-
    findall(
        word(ID, Family, LanguageID, Gloss, CognatesetID, Transcription),
        (   lex(_, ID, LanguageID, _, _, _, _, Family, ConceptID, Gloss, CognatesetID, _, Transcription),
            \+ CognatesetID = ''
        ),
        ConceptSet).
...
random_concept_set_with_cognates(ConceptSet) :-
    findall(
        [Family, ConceptID], 
        (   lex(_, _, _, _, _, _, _, Family, ConceptID, _, CognatesetID, _, _),
            \+ CognatesetID = ''
        ),
        FamilyConcepts),
...
system_prompt('You are an expert in phonetics, phonology, and diachronic linguistics.  You are a master at applying the Comparative Method.').
